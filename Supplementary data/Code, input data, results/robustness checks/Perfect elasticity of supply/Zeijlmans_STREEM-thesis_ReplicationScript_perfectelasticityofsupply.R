...
# Import the data
data <- read.csv("Zeijlmans_InputData_noeta.csv", sep = ",")  
...
scenario_result <- function(taxlevel, taxtype, data){
  ...
  P_1_cons <- data$P_eq_0 + tau
  
  # Calculate new consumer prices
  P_1_prod = data$P_eq_0
  delta_P_abs = tau
...
#carbon tax table
carbonsteps <- c(seq(0.01, 15, by=0.01), seq(15.01, 30, 0.01), seq(30.1, 450, 0.1) ) #set iterations
i<-0
...
#slaughter tax table
slaughtersteps <- c(seq(0.001, 1, by=0.001), seq(1.001, 3, 0.001), seq(3.01, 90, 0.01) )
i <- 0
...
TaxRevenueDF <- data.frame(TR = seq(1, 350, 10)) #This one is for plotting because TR=1 is the first sensible result.
TaxRevenueDFround <- data.frame(TR = seq(0, 350, 10)) #This one is for the table because round values are better than 11, 21, 31, etc.
...
plot(TaxRevenueDF$TR, TaxRevenueDF$SCAUThreshold, type = "b", pch = 19, col = "black", 
     ...
     main = "Threshold values for SCAU under\nperfect elasticity of supply",
     family = "A",
     font = 1
     )
text(x = max(TaxRevenueDF$TR) - 190, 
...
text(x = min(TaxRevenueDF$TR) + 280, 
     y = min(TaxRevenueDF$SCAUThreshold) + 0.008, 
